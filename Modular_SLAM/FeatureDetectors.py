...
class RANSAC(object):
	...
	#Randomly sample data readings within neighborhood of this laser data reading
			shortList=[(a,d) for (a,d) in FullScan if seedAngle-self.neighbourhood < a < seedAngle+self.neighbourhood]
	...
	#Determine how many laser data readings lie within self.error_range meters of this best fit line       
			pointsOnLine=[]
			for u in UnAsc:
				x1=u[0]
				y1=u[1]
	...
	#If the number of laser data readings on the line is above some consensus self.consensus
...
class HOUGH(object):
	...
	def __init__(self,maxRange,radialResolution,angularResolution,threshold):
		self.maxRange = maxRange	#Max range of Lidar to be used for defining blank image (cm). 
		self.radialResolution = radialResolution	#Resolution for line detection 
...
class CORNER_EXTRACTOR(object):
	...
	def __init__(self,maxRange,radialResolution,angularResolution,threshold):
  	...
  	def get_landmarks(self, angles, distances):
		...
		image = GaussianBlur(image, (11,11), 0)
		features = goodFeaturesToTrack(image,20, 0.8, 15)
		return self.features_to_coords(features, image.shape[0], 
			                           points[1], points[-1])
  	...
  	def features_to_coords(self, features, length, firstPoint, lastPoint):
  		landmarks = []
  		features = features - length/2
		for feature in features:
			x = feature[0][0]/float(100)#/float(length/2)*self.maxRange
			y = -feature[0][1]/float(100)#/float(length/2)*self.maxRange
			# Remove the corners that will always be detected at -30 and 210 degrees (Endpoints of Hokoyu)
			if abs(firstPoint[0]-x) < 5 and abs(firstPoint[1]-y) < 5:
				#print "First point detected"
				pass
			elif abs(lastPoint[0]-x) < 5 and abs(lastPoint[1]-y) < 5:
				#print "Last point detected"
				pass
			else:
				landmarks.append(((x,y),1))
		return landmarks

class IMAGE_LINES(object):
	"""Class to find vertical lines in on-board camera images""" 

	def __init__(self,kernalSize,thresholdingBlockSize,thresholdingConstant,edgeThreshold1,edgeThreshold2,edgeApertureSize,angleRange,distanceRange,lineThreshold,fieldOfView,angularError):
		
		#parameters for median blur 
	  	self.kernalSize = kernalSize							#Aperture linear size; it must be odd and greater than 1, for example: 3, 5, 7 ...

		#adaptiveThreshold parameters
		self.thresholdingBlockSize = thresholdingBlockSize		#Size of a pixel neighborhood that is used to calculate a threshold value for the pixel: 3, 5, 7, and so on.
		self.thresholdingConstant = thresholdingConstant		#Constant subtracted from the mean or weighted mean 

		#Canny edge detector parameters
		self.edgeThreshold1 = edgeThreshold1					#First threshold for the hysteresis procedure
		self.edgeThreshold2 = edgeThreshold2					#Second threshold for the hysteresis procedure
		self.edgeApertureSize = edgeApertureSize				#Aperture size for the Sobel() operator
		
		#HOUGH transform parameters
		self.angleRange = angleRange 							#Angle resolution of the accumulator in radians
		self.distanceRange = distanceRange 						#Distance resolution of the accumulator in pixels
		self.lineThreshold = lineThreshold 						#Accumulator threshold parameter. Only those lines are returned that get enough votes (>threshold)

		self.fieldOfView = fieldOfView							#Horizontal Field of View
		self.angularError = angularError						#EKF Error variable for Covariance
	...
	def get_landmarks(self,image):
		...
		th2 = adaptiveThreshold(gray,255,ADAPTIVE_THRESH_MEAN_C,THRESH_BINARY,self.thresholdingBlockSize,self.thresholdingConstant)
		#Find edges in image
		edges = Canny(th2,self.edgeThreshold1,self.edgeThreshold2,apertureSize = self.edgeApertureSize)
		#Find lines in image
		lines = HoughLines(edges,self.distanceRange,self.angleRange,self.lineThreshold)
