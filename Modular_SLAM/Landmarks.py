...
class LANDMARKS(object):
	...
	@staticmethod
	def new_landmark(landmarkType,landmarkPosition,landmarkError,robotPosition):
		...
		if landmarkType == "Wall":
			return WALL(landmarkPosition,landmarkError,robotPosition)
		if landmarkType == "Corner":
			return CORNER(landmarkPosition,landmarkError,robotPosition)
	pass
...
class CORNER(POINT_FEATURES):
	"""Class for corner objects"""
	def __init__(self,cornerPosition,error,robotPosition):
		self.position = self.cart_to_world_frame(cornerPosition,robotPosition)
	...
	def check_if_in_map(self,ExistingList):
		...
		for Obj in ExistingList:
			if Obj.__class__.__name__ == "CORNER":
				dx,dy = abs(self.position[0] - Obj.position[0]), abs(self.position[1] - Obj.position[1])
				dist = sqrt(dx**2 + dy**2)
				check_distance = min(check_distance, dist)
				if dist < best_distance:
					best_distance = dist
					best_match = Obj
		if 0.1 < check_distance < 0.7:
			best_match = None
		return best_match

	def cart_to_world_frame(self,point, robotPosition):
		# Why do I need to do this? Something to do with frame between robot and world I think
		angle = -robotPosition[2] + radians(90)
		dx = point[0]*cos(angle) + point[1]*sin(angle)
		dy = -point[0]*sin(angle) + point[1]*cos(angle)
		return array([dx + robotPosition[0], dy + robotPosition[1]])
