...
class CORNER_EXTRACTOR(object):
	...
	def __init__(self,maxRange,radialResolution,angularResolution,threshold):
  	...
  	def get_landmarks(self, angles, distances): 
		...
		image = GaussianBlur(image, (15,15), 0)
		features = goodFeaturesToTrack(image,20, 0.8, 15)
		for x, y in features[:,0]:
			circle(image, (x, y), 2, (255,255,255), -1)
		circle(image, (500, 500), 2, (255,255,255), -1)
		landmarks =  self.features_to_coords(features, image.shape[0], 
			                                 points[1], points[-1])
  	...
  	def features_to_coords(self, features, length, firstPoint, lastPoint):
		...
		for feature in features:
			x = feature[0][0]#/float(length/2)*self.maxRange
			y = -feature[0][1]#/float(length/2)*self.maxRange
