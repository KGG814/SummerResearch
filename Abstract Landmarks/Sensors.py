...
class LIDAR(object):
	...
	def __init__(self,minLidarRange,maxLidarRange,lidarOffset,rangeError,
	             bearingError,VisualizerObject):
    ...
    #Base error for all landmarks observed by this lidar
		self.lidarError    = diag([rangeError,bearingError])	
	...
	def observe(self,robotPosition):
		...
		for Obj in self.detectors:
		  #Find out the type of landmark the detector is going to return
			landmarkType = Obj.landmarkType		
			#Get a list of landmarks in form of tuples [(position, certainty),...]
			landmarks = Obj.get_landmarks(angles,distances)		
			...
			for (landmarkPosition,certainty) in landmarks:
				...
				# TODO
				landmarkObject = LANDMARKS.new_landmark(landmarkType,
				                 landmarkPosition,landmarkError,
				                 lidarPosition, (angles, distances))
